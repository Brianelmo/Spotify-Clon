...
function PlayListZone() {
  ...
  const setLocation = () => {
    if (location.pathname === "/home") {
      return (
        <h2 className="text_home text-white text-2xl font-extrabold">
          {TEXT_HOME}
        </h2>
      );
    } else if (location.pathname === "/search") {
      return (
        <h2 className="text_search text-white text-2xl font-extrabold">
          {TEXT_SEARCH}
        </h2>
      );
    }
  ...
  return (
    <div className="rounded playlistzone">
      ...
      <section className="text-white text-2xl font-semibold p-5">
        {PlayListGralservice()}
        <div
          className={`CardPlaylistZone ${searchKey === "" ? "ocultar" : ""}`}
        >
